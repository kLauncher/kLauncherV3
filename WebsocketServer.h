...
public:
	...
	WebsocketServer();
	void run(int port, HANDLE hListenEvent);
	...
	void sendMessage(ClientConnection conn, const string& messageType, const json& arguments);

	//Sends a message to all connected clients
	//(Note: the data transmission will take place on the thread that called WebsocketServer::run())
	void broadcastMessage(const string& messageType, const json& arguments);

protected:
	static json parseJson(const string& json);
	static string stringifyJson(const json& val);
	...
	map<string, vector<std::function<void(ClientConnection, const json&)>>> messageHandlers;
};
